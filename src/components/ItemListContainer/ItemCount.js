...
const ItemCount = ({cupos, nombre}) => {
  ...
  return (
    <div className="mt-4">
      ...
      <button type="button" onClick={() => addToCart(items)} disabled={disabledButton} className={`w-full py-2 my-2 bg-blue-600 border text-white font-semibold col-span-1 border-gray-200 rounded-2xl disabled:opacity-50 ${disabledButton === true ? 'cursor-not-allowed' : 'hover:bg-blue-700'}`}>Agregar al carrito</button>
      {
        modal && (
